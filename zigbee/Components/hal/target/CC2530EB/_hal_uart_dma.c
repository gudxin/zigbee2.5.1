...
// Incompatible redefinitions between the 2 UART driver sub-modules:modified by lu
#undef PxSEL
#undef UxCSR
#undef UxUCR
#undef UxDBUF
...
#if (HAL_UART_DMA == 1)
  PERCFG &= ~HAL_UART_PERCFG_BIT;    //~0x01;// Set UART0 I/O to Alt. 1 location on P0.
#else
  PERCFG |= HAL_UART_PERCFG_BIT;     // Set UART1 I/O to Alt. 2 location on P1.
#endif
  PxSEL  |= HAL_UART_Px_RX_TX;       // 0x0c;//Enable Tx and Rx on P1.
  ADCCFG &= ~HAL_UART_Px_RX_TX;      //~0x0c;// Make sure ADC doesnt use this.
