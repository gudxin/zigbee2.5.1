 ...
 * @return  none
 *****************************************************************************/
static void HalUARTInitISR(void)//modified by lu
{
...
#else
  PERCFG &= ~0x02;    //把串口一配置到P0.4_P0.5 
#endif
  P0SEL  |= 0x30;     //选择为还口TX与RX
  ADCCFG &= ~0x30;    //HAL_UART_Px_RX_TX; 0xc0;     // Make sure ADC doesnt use this.
  U1CSR = CSR_MODE;                  //  串口模式
